...
| 317 | 19 (was 18) | layout | Table hides text — slide_llm_judge | done — left bullets at 24pt in 5.5" col were 50–66 chars each, wrapping to 8 lines (~3.0–3.4") and overflowing the 2.1" frame into the results table at T=4.55. Trimmed 4 bullets → 3 short single-line bullets so rendered text fits inside frame; no more collision with table |
| 326 | 34, 35 | layout | Text misplaced on both | done (slide 34) / verified clean (slide 35). Slide 34 (slide_nbest_v3_judge_paired_tests): cap_methods at T=6.20 H=0.80 BOT=7.00 overlapped the run-label footer at T=6.55 by 0.45". Reduced cap_methods H 0.80→0.30 → BOT=6.50, 0.05" gap before footer. Slide 35 (slide_mbr_decision): all shapes audited (sub / tbl 2.00–4.52 / dec rect 4.75–6.55 / dec bullets 5.30–6.40) — within safe zone, no overlaps; no change applied |
