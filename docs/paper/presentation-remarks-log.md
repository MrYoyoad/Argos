...
| 275 | 19 | `slide_llm_judge_30` (`slides_evaluation.py`) | Col2 `"30 (stratified sample)"` (22 chars) wrapped in col2=3.0" (eff=2.7"). Fixed col_widths [2.5,3.0]→[2.0,3.5]. | done |
| 276 | 52 | `slide_three_thresholds` (`slides_evaluation.py`) | Subtitle 2-line text (h=0.5") overflowed into table at CT+0.6. Fixed subtitle h→0.45" (single line), table y→CT+0.55. Col widths [2.2,1.8,3.1,4.6]=11.7"≠CW; corrected to [2.30,1.90,3.20,4.73]=12.13". | done |
| 277 | 56 | `slide_agreement_vs_conf_information` (`slides_evaluation.py`) | 4 bullets at 24pt in h=1.60" overflowed footer. Bullet box h→1.80"; footer y→6.65", h→0.40". | done |
| 278 | 57 | `slide_client_trust_calibration` (`slides_evaluation.py`) | Subtitle h=0.5" (2 lines, 18pt) overflowed table at CT+0.6. Fixed h→0.7"; table y→CT+0.75. Col2 "n trusted" (bold ≈1.16") clipped in 1.4" (eff=1.1"); widened to 1.6". Col6 header "% clearly conveyed in trust" (27 chars) wrapped; shortened to "% Y+P in trust". | done |
| 279 | 75 | `slide_arabic_roadmap` (`slides_future.py`) | Timeline text at y=5.87" placed 0.13" above timeline box (starts at y=6.00"). Fixed to y=6.05". | done |
...
|---|----------|----------|--------|--------|
| 305 | 2, 3 (what_was_done) | content | MBR framing unclear, "p" framing bad. Rewrite: beam aggregation used + worth it. Positive: model works amazingly, can be trusted. Emphasize Docker build, made it work perfectly from paper/idea/UI. More like original what_was_done tone. | done — slide 2 reframed: paper→production headline, "20 hypotheses combined" instead of MBR jargon; slide 3: "Model works amazingly" + "System tells you when to trust it" both bold |
...
| 308 | 7 | layout | Table hides text, many occlusions | done — slide_visemes (slide 7) had tbl2 (word-pair table) overlapping the bottom callout: default row_height=0.38 × 5 rows = 1.90", starting at CT+3.05=4.50 → ended at 6.40, callout starts at 6.30 (0.10" overlap). Fix: row_height 0.38→0.32 on both tables → tbl2 ends at 6.10, 0.20" gap to callout. tbl1 also tightened for consistency |
...
| 311 | pipeline | layout | Pipeline figure text looks horrible and occluded + cut in the middle | done — slide_17 split (see #316). Subsequently extended: slide_17_b grew 4→6 stages adding 7a Aggregate (MBR n-best, May 1) + 7b Confidence (per-word p₁ + α, Apr 30) between LLM Decode and Outputs. Box width 1.95→1.80 with tighter gaps so 6 boxes fit within 13.33" slide. Teal "Engineered May 2026" highlight on the new pair |
...
| 317 | 18 | layout | Table hides text | done — slide_llm_judge bullets at 24pt in 5.5" col were 50-66 chars each, wrapping to ~8 lines and overflowing 2.1" frame into the results table. Trimmed 4 bullets → 3 short single-line bullets; no more collision |
...
| 322 | 25 | content | Not important — only the example matters. Needs confidence score so user understands whether they would be fooled | done — slide_is_intro_a (slides_research.py L143-200): Length Ratio card grown 1.45→1.55"; its italic "Hallucinated 2.8×" example replaced by a worked example strip — street_photo ref/hyp pair + 5 colored badges (WER 56%, WWER 53%, LR 1.05 in CORAL/LGRAY; conf 0.70 + "Salvage — inspect, don't trust blindly" in GOLD). Audience now sees a concrete pair AND that confidence flags it for review. |
...
| 326 | 34, 35 | layout | Text misplaced on both | done — slide 34 (slide_nbest_v3): cap_methods H 0.80→0.30 (was overlapping run-label footer by 0.45"); slide 35 (slide_mbr_decision): geometry verified clean, no change needed |
...
| 329 | 39 | content | Extra figure not needed — remove it | done — was slide_08 (Failure Mode Taxonomy bar chart, slide 38 in current deck). Right-side image (FAILURE_TAXONOMY_HIERARCHY) was redundant with the bar chart — removed earlier in batch 23 (commit 0df261f); bar chart is now full-width |
| 330 | 39, 40 | content | Why 39 AND 40? Slide 40 not written properly together with 39 | done — slide_failure_deep_1a (39, taxonomy text examples) and slide_failure_live_demo (40, live videos) were disconnected. Fixes: (1) slide 39 title "(1/2): Highest Impact First" → "Top 3 by Impact" (was implying a non-existent part 2/2 in this deck); (2) slide 39 footer "continued in part 2/2" → TEAL "→ Live video examples of Right-Topic and Hallucination on the next slide"; (3) slide 40 title "Failure Modes — Live Examples" → "Failure Modes — Live: Right-Topic-Names-Lost & Hallucination" (explicit callback to slide 39's modes) |
...
| 333 | 50 | layout | Text hides other stuff | done — slide_three_thresholds verified clean, all shapes within safe zone, no real overlap (only 0.05" title-bbox proximity artifact, no text collision) |
| 334 | 52 | layout | Math hiding of boxes/tables | done — slide_band_reliability_by_niv: image was overlapping bot text by 0.33" and sub by 0.30". Image height 4.60→4.25, sub height 0.80→0.45. Plot still 9.0"×4.25" |
...
| 336 | 54 | layout | Plot too small + labels in plot too small + text in the way of plot/table | done — slide_agreement_aware_bands WHY inner-text height 1.65→1.20 (was overflowing rect by 0.15"). Note: this slide has no plot; the "plot too small" portion of the remark may have meant slide 52 (band_reliability_by_niv plot, which was enlarged in #334) |
...
| 347 | — | content | More slides needed from new client PPTX and original AMOSI. Missing focus: engineering done, lot of work, etc. | done — added 3 engineering recap slides at positions 68/69/70 (cap of §5 before FUTURE divider): slide_engineering_summary (6-card "What We Engineered" 3×2), slide_engineering_pipeline_modules (11 lib/ shell modules in 3×4 grid color-grouped by phase), slide_engineering_what_shipped (5-card timeline Apr 30–May 3) |
...
| 349 | 18, 19, 64–67 | layout | Visual audit fixes from rendered PNGs | done — (1) slide_literature_metrics_problem (18): right-card "Same WER ~50%" header h 1.0→0.40 + Pt(24)→Pt(20), pair-1 py 1.05→1.15 (no longer overlaps pair-1 label). (2) slide_llm_judge (19): results table text_size 24pt→20pt + row_height 0.40"→0.36" so PowerPoint auto-grow no longer pushes table past 6.55 into the v1-judge footer label at 6.6. (3) slides 64–67 demo metrics_line shortened to fit one 18pt line in 9.53"-wide box (was wrapping into the video below): obama_salvage "WER ~22% / IS ~3.5 / mean_prob 0.920 (TRUST under T_safe=0.82)"; obama_strip "WER 43% / IS 2.66 (Salvage) / mean_prob 0.739 / joint rule"; judge_entity "WER 18% / IS 4.55 / mean_prob 0.624 (Strip)"; judge_vocab "WER 52% / IS 3.02 / mean_prob ~0.78 (Salvage)" |

## 2026-07-13 — July client meeting prep (Egla-Kafe two-deck build)

User request: plan the final client presentation before departure (~1 hr, Egla-Kafe client only, decision-makers + operators; goals: show the Egla-Kafe data well, prove value, secure continuation funding; succession deliberately part of the ask). Approach chosen: **two artifacts**.

| # | Item | Disposition |
|---|------|-------------|
| 350 | Deck 1 refresh (data story) | done — `Argos_VSP_EglaKafe_20260713.pptx` (10 slides): title date → July 2026; confident-subset headline 75–90% → **70–92%** (matches the gate ladder + its own footer); `slide_whats_next` dropped (roadmap moved to Deck 2 to avoid presenting it twice). All numbers re-verified against final findings.md — plots/clips were already built from final data (June 25). |
