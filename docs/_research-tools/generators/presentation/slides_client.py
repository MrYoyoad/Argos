...
def slide_client_engineering_journey(prs):
    ...
    set_notes(slide, (
        ...
        "not what was broken."
        "\n\n"
        "ROUND 5.7 ADD — Mission 6 (beam aggregation) shipped May 1, "
        "2026: five aggregation methods (MBR, score-vote, conf-vote, "
        "safe, xseg-merge) gated by VSP_NBEST=1, default OFF. "
        "Partial evaluation on 107 segments (tuning set): hyp_vote_conf "
        "reduced WER from 59.35% to 57.20% (−2.15pp absolute, −3.6% "
        "relative) AND lifted IS by +0.029 and NIV-Y+P by +0.9pp. "
        "Wins on every metric. "
        "\n\n"
        "If a co-partner pulls on 'how does aggregation interact with "
        "confidence?': the raw per-word agreement scores from voting "
        "methods are inflated (median 1.00 vs 0.86 for top-1) because "
        "beams aren't independent samples — they share encoder/decoder "
        "weights and prefix histories. Agreement is a *ranking* signal, "
        "not a posterior. Documented fix is temperature scaling on a "
        "held-out set (Guo et al. 2017, Desai & Durrett 2020). We ran "
        "that experiment May 1: after T-scaling, hyp_vote_conf has the "
        "BEST calibrated ECE (0.064) — better than top-1 (0.086). So "
        "WER reduction AND calibration are both achievable with the "
        "voting method, plus one one-time temperature parameter. "
        "\n\n"
        "Full 1,497-segment evaluation is running right now in "
        "english_full_nbest_eval/. Numbers will refresh; framing won't. "
        "Sources: docs/beam-search/n_best_implementation.md "
        "(headline + caveats), tuning_results/exp_nbest_validation/"
        "calibration.json (T-scaling results), aggregator_method_summary"
        ".json (WER table). "
        "\n\n"
        "If they ask 'when does this become default?': after the full "
        "1,497-segment run validates, after we've decided whether to "
        "ship vote_conf (best WER) or MBR (best raw confidence "
        "calibration without T-scaling). Probably both: vote_conf for "
        "the transcript, MBR's confidence inheritance for the trust "
        "signal."
    ))
    return slide
