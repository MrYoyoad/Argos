...
if [ "$SEGMENT_ONLY" = "1" ]; then
  ...
  if [ "$SEGMENTATION_ENABLED" = "1" ]; then
    ...
    # Copy whole videos to fast_segments directory for review
    # Use segment naming format for compatibility with UI
    copied_count=0
    ...
    for video_file in "${RAW_DIR}"/*.mp4 "${RAW_DIR}"/*.mkv "${RAW_DIR}"/*.webm "${RAW_DIR}"/*.mov "${RAW_DIR}"/*.avi "${RAW_DIR}"/*.m4v; do
      if [ -f "$video_file" ]; then
        ...
        cp "$video_file" "${FAST_SEG_DIR}/${video_id}_00_000000_999999.${video_ext}"
