...
# OFF by default in the container build (VSP_ENHANCE=0) — opt-in feature,
# not yet validated end-to-end on client hardware. Enable with VSP_ENHANCE=1.
############################################
VSP_ENHANCE="${VSP_ENHANCE:-0}"
if [ "$VSP_ENHANCE" = "1" ]; then
  echo ">>> [0.6] Enhancing video quality (CLAHE + brightness normalization + sharpening)"
  ENHANCE_SCRIPT="${SCRIPT_DIR}/scripts/pipeline/enhance_videos.py"
  ...
  echo ">>> [0.6] Video enhancement disabled (VSP_ENHANCE=0; set VSP_ENHANCE=1 to enable)"
fi
echo
