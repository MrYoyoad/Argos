...
| `report.html` shows green/yellow/red colors, not blue/orange/purple | Wrong / stale `make_report.py` got into the image | Image was built from a stale container_payload_20260507. Apply the latest patch via `apply_update.sh`, or rebuild the image. |
...
| Pipeline finishes but `$HOME/vsp-output/<ts>/` is empty | The export hook didn't fire (maybe an old image without the hook) | Check the image was built from the May 2026 container_payload_20260507 (with the VSP_OUTPUT_DIR hook in `lib/outputs.sh`). Older images don't honor the env var. |
