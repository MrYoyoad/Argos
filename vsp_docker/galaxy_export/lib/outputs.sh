...
" 2>&1 || log_warn "Lip crop copy failed (non-critical)"
  ...
  # ---- VSP_OUTPUT_DIR export hook (container-side bind-mount target) ----
  # When VSP_OUTPUT_DIR is set (typically by the desktop launcher in the
  # air-gapped container), copy user-facing final artifacts to that dir so
  # they land outside the container in a single place. Additive: when unset
  # (EC2 callers), nothing happens.
  if [ -n "${VSP_OUTPUT_DIR:-}" ]; then
    if [ ! -d "$VSP_OUTPUT_DIR" ]; then
      log_error "VSP_OUTPUT_DIR=${VSP_OUTPUT_DIR} does not exist"
      return 1
    fi
    if [ ! -w "$VSP_OUTPUT_DIR" ]; then
      log_error "VSP_OUTPUT_DIR=${VSP_OUTPUT_DIR} is not writable"
      return 1
    fi

    log_info "Exporting user-facing artifacts to ${VSP_OUTPUT_DIR}"
    local copied=0

    # report directory: contains report.html, report.csv, intelligibility_scores.csv,
    # aggregated.json, confidence_breakdown.html, etc.
    if [ -d "$report_dir" ]; then
      cp -r "${report_dir}/." "${VSP_OUTPUT_DIR}/"
      copied=$((copied+1))
    fi

    # Burned videos (gated on VSP_FULL_OUTPUTS=1 above; copy what's there).
    if [ -d "$burn_dir" ] && [ -n "$(ls -A "$burn_dir" 2>/dev/null)" ]; then
      mkdir -p "${VSP_OUTPUT_DIR}/burned_videos"
      cp -r "${burn_dir}/." "${VSP_OUTPUT_DIR}/burned_videos/"
      copied=$((copied+1))
    fi

    # Pipeline + decode logs (lets post_install_check grep them).
    for log_name in pipeline.log decode.log; do
      if [ -f "${archive_root}/${log_name}" ]; then
        cp "${archive_root}/${log_name}" "${VSP_OUTPUT_DIR}/"
        copied=$((copied+1))
      fi
    done

    # K-means model (single file in $VSP).
    if [ -f "${vsp_dir}/${data_name}_kmeans_200.bin" ]; then
      cp "${vsp_dir}/${data_name}_kmeans_200.bin" "${VSP_OUTPUT_DIR}/"
      copied=$((copied+1))
    fi
