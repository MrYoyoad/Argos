...
# Primary wait: training PID must exit AND GPU must be free
# (checkpoint_best.pt appears mid-training, so don't use it as trigger)
while true; do
    ...
    # Break only when PID is dead AND GPU is free
    if ! ${PID_ALIVE} && [ "${GPU_PROCS}" -eq 0 ]; then
        echo "[$(date)] Training PID stopped and GPU is free"
        # Double-check after brief wait
        sleep 30
        GPU_PROCS=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null | wc -l)
        if [ "${GPU_PROCS}" -eq 0 ]; then
            echo "[$(date)] Confirmed: ready for evaluation"
            break
        fi
    fi
    ...
    if ${PID_ALIVE}; then
        LAST_UPDATE=$(grep -o '"num_updates": "[0-9]*"' "${EXPB_CKPT_DIR}/../hydra_train.log" 2>/dev/null | tail -1 | grep -o '[0-9]*')
        echo "[$(date)] Training active — update ${LAST_UPDATE:-?}/3000, GPU procs: ${GPU_PROCS}"
    else
        echo "[$(date)] PID stopped but GPU still has ${GPU_PROCS} process(es), waiting..."
    fi
